...
!!! note "公開状況（制作中）"
    現在、**第1回〜第70回（第1部〜第6部）を公開中**です。Python の基礎（第1〜20回）、数値計算・NumPy（第21〜30回）、データ処理・pandas（第31〜45回）、データ可視化（第46〜55回）、化学に特化した**ケモインフォマティクス・RDKit（第56〜70回）**がそろっています。統計の R（第7部）、機械学習（第8部）、レポート作成（第9部）は順次追加していきます。各回は単独でも動くように書いています。
